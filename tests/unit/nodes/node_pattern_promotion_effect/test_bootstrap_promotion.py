...
@pytest.mark.unit
def test_bootstrap_candidate_rejected_zero_recurrence() -> None:
    """Candidates with zero recurrence should NOT be promotable via bootstrap.

    Threshold lowered to 1 in OMN-5500 fix — only 0 is rejected.
    """
    pattern = _make_pattern(confidence=0.9, recurrence_count=0, distinct_days_seen=2)
    assert meets_candidate_to_provisional_criteria(pattern) is False


@pytest.mark.unit
def test_bootstrap_candidate_accepted_at_recurrence_one() -> None:
    """Candidates with recurrence_count=1 should be promotable (threshold lowered from 2)."""
    pattern = _make_pattern(confidence=0.9, recurrence_count=1, distinct_days_seen=1)
    assert meets_candidate_to_provisional_criteria(pattern) is True


@pytest.mark.unit
def test_bootstrap_candidate_rejected_zero_distinct_days() -> None:
    """Candidates with zero distinct_days should NOT be promotable via bootstrap.

    Threshold lowered to 1 in OMN-5500 fix — only 0 is rejected.
    """
    pattern = _make_pattern(confidence=0.9, recurrence_count=3, distinct_days_seen=0)
    assert meets_candidate_to_provisional_criteria(pattern) is False
...
def test_bootstrap_boundary_at_exact_threshold() -> None:
    ...
    pattern = _make_pattern(confidence=0.8, recurrence_count=1, distinct_days_seen=1)
    assert meets_candidate_to_provisional_criteria(pattern) is True
...
def test_bootstrap_boundary_just_below_confidence() -> None:
    ...
    pattern = _make_pattern(confidence=0.79, recurrence_count=1, distinct_days_seen=1)
    assert meets_candidate_to_provisional_criteria(pattern) is False
